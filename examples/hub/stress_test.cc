  ...
  void run() {
    ...
    for (int i = 0; i < numberOfConnections_; ++i) {
      std::shared_ptr<PubSubClient> client(
        new PubSubClient(&loop, InetAddress(hostip_, port_), string(name_.data()))
      );
...
private:
    ...
    if (client->connected()) {
      if (!FLAGS_pub) {
        for (auto it = topics_.begin(); it != topics_.end(); ++it) {
          client->subscribe(
            muduo::string(it->data()),
            boost::bind(&StressTest::onSubscription, this, _1, _2, _3)
          );
        }
      } else {
        sendMessages(client);
  ...
  void sendMessages(PubSubClient* client) {
    ...
    for (auto& topic : topics_) {
      client->publish(
        muduo::string(topic.data()), muduo::string(message.data())
      );
    }
  }
